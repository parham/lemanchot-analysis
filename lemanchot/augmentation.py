...
def generate_random_mask(
    ...
    points = list(
        map(lambda p: (int(p[0] * width) + bx, int(p[1] * height) + by), points))
    # Create the mask
...
def generate_augmented_texture(
    ...
    row_count = 20
    col_count = 20
    row = np.append(np.array(range(row_count)) *
                    (shape[0] // row_count), shape[0]-1)
    column = np.append(np.array(range(col_count)) *
                       (shape[1] // col_count), shape[1]-1)
    ...
    for index in range(num_texture):
        texture = np.resize(
            textures[index].cpu().detach().numpy(), background.shape)
        target = np.resize(
            targets[index].cpu().detach().numpy(), img_target.shape)
