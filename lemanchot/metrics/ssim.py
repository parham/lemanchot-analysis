...
from lemanchot.metrics.core import (
    Function_Metric,
    assert_image_shapes_equal,
    metric_register,
)
...
class SSIM(Function_Metric):
    def __init__(self, config):
        super().__init__(func=ssim, config=config)


def ssim(
    org_img: np.ndarray, pred_img: np.ndarray, max_p: int = 255, **kwargs
) -> float:
    """
    ...
    res = structural_similarity(
        ...
        pred_img,
        data_range=max_p,
        channel_axis=0 if org_img.ndim == 3 else None,
    )
    return {"ssim": res}
...
def issm(org: np.ndarray, pred: np.ndarray, **kwargs) -> float:
    ...
    org_img = (
        org if len(org.shape) > 2 else org.reshape((org.shape[0], org.shape[1], 1))
    )
    pred_img = (
        pred if len(pred.shape) > 2 else pred.reshape((pred.shape[0], pred.shape[1], 1))
    )
...
class ISSM(Function_Metric):
    def __init__(self, config):
        super().__init__(func=issm, config=config)
