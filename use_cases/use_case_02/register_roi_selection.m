...
for findex = 1:length(vis_files)
    ...
    mfile = fullfile(root_dir, 'masks', tfname);
    % Check if all modalities exist
    if ~isfile(vfile) || ~isfile(tfile) || ~isfile(mfile)
        fprintf('Skipped!\n')
        continue;
    else
        fprintf('Done!\n')
    ...
    [aligned_ir, aligned_mask] = phm.Fusion(viz, ir, mask, tcp, vcp);

    new_ir = uint8(aligned_ir) .* uint8(aligned_mask);
    new_viz = uint8(viz) .* uint8(aligned_mask);

    imshow(imfuse(new_viz, new_ir, 'blend'), [])
    axis on;
    title('Original Grayscale Image', 'FontSize', fontSize);
    set(gcf, 'Position', get(0,'Screensize'));
%     hFH = drawfreehand();

    okind = find(aligned_mask > 0);
    [ii,jj] = ind2sub(size(aligned_mask), okind);
    ymin = min(ii); ymax = max(ii); xmin = min(jj); xmax = max(jj);
    % Crop the thermal image to bound the selected ROI
    ir_roi = imcrop(new_ir, [xmin,ymin,xmax-xmin+1,ymax-ymin+1]);
    % Crop the visible image to bound the selected ROI
    viz_roi = imcrop(new_viz, [xmin,ymin,xmax-xmin+1,ymax-ymin+1]);
    % Crop the mask image to bound the selected ROI
    mask_roi = imcrop(aligned_mask, [xmin,ymin,xmax-xmin+1,ymax-ymin+1]);
    
    fprintf('Saving the fused file ... %s\n', fused_savefile);
    save(fused_savefile, 'ir_roi', 'viz_roi', 'mask_roi');
